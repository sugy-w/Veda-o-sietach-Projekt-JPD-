...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        \hline \textbf{Model siete}         & \textbf{Hrany $|E|$} & \textbf{Clustering ($C$)} & \textbf{Priem. cesta ($L$)} \\
        \hline \textbf{Reálna letecká sieť} & \textbf{18 859}      & \textbf{0.4918}           & \textbf{3.9585}             \\
        Erdős–Rényi (Náhodný)               & 18 859               & 0.0041                    & 3.5708                      \\
        Barabási–Albert (Huby)              & 18 834               & 0.0201                    & 3.1644                      \\
        Konfiguračný model                  & 17 893               & 0.0732                    & 3.3343                      \\
        \hline
    \end{tabular}
\end{table}
...
vysoko efektívny systém. Pre hlbšiu validáciu sme vypočítali aj Telesfordov koeficient \textbf{$\omega \approx 0.21$}. Hodnota blízka nule potvrdzuje,
že sieť sa nachádza v „zlatom strede“ medzi prísne usporiadanou mriežkou a úplným chaosom.

V praxi to znamená, že napriek obrovským fyzickým vzdialenostiam žijeme vo „svete štyroch letov“. Priemerná dĺžka cesty $L \approx 3.96$ naznačuje, že
globálna dostupnosť je zabezpečená pozoruhodne malým počtom krokov.
...
\begin{itemize}
    ...
    minimálne (v rozsahu $3.1$ až $3.9$). To naznačuje, že nízky počet prestupov je \textbf{univerzálnou vlastnosťou} riedkych sietí s daným počtom
    ...
    \item \textbf{Efektivita BA modelu:} Barabási–Albertov model vykazuje najnižšiu priemernú cestu ($3.16$). To naznačuje, že ak by sa svet riadil
